...
To prepare the release of a public API version, at least two API versions must be (pre-)released as follows:

* at M3: after one or more alpha API versions, the release-candidate API version implements the scope for the target public API version. This pre-release is agreed stable for API implementation and functional testing, and aligned with the release-candidate versions of Commonalities and ICM (M1).
* at M4: fter one or more release-candidate API versions, the public API version is ready for the meta-release. The public API version is aligned with the public versions of Commonalities and ICM (M2).

An API Sub Project can release as many alpha and release-candidate API versions as useful for API development and testing. In between releases, the API version is set to "wip" to avoid erroneous usage of the API.

An API Sub Project shall create (pre-)releases for its API versions as follows:
...
* Before M3 and M4, the release PR is prepared. It shall minimally update the API version, the API readiness checklist and the changelog.
...
* the expected (pre-)releases for alpha, release-candidate and public API versions need to be provided at the respective M3 and M4 milestones
...
* Once the required stability is reached, create the release PR. A release PR provides only the following changes: 
  ...
  * update the [CHANGELOG.md]() in the home of the repository. Add a new section at the top for the release and each API version with the following content:
...
* alpha API versions are released before M3
* the first release-candidate API version is released at M3
* release-candidate API versions ae released between M3 and M4
* the public API version is released at M4
