...
- **Owned and governed by the API Provider.** Deployment ICM-compatibility is a contractual matter between API Provider and API Consumer, communicated by the Provider through onboarding documentation or other means (for example via OIDC discovery). The mandatory `info.description` text in every CAMARA API spec explicitly states that "the specific authorization flows to be used will be agreed upon during the onboarding process."
- **Identified at deployment time.** An API Consumer determines the applicable ICM version's deployment info from their API Provider onboarding contract or other artifacts; the API version alone does not pin a specific ICM version on the deployment side.
...
The table below defines the repeating pattern of actions that applies at **every** Signal/Sync pair. 

At any Signal meta-release, either a new **major** ICM version, or an (additive) **minor** ICM version is released, impacting the actions of the various stakeholders to maintain ICM-compatibility as described in the below table. NOTE: a **patch** ICM version release can happen at any time and replaces the ICM version that the patch is applied to.

| Meta-release | Event | API designer | API Provider | API Consumer |
|---|---|---|---|---|
| **Signal N** (major ICM version case) | A new **major** ICM version `vX+1.0.0` is released in **Supported** state | Assess ICM-compatibility of APIs with this new ICM version: record cross-major ICM compatibility or plan new API version with vX+1.0.0 if the API is not ICM-compatible | Assess, plan, and announce deployment of ICM `vX+1.0.0` | — |
...
The ICM version lifecycle transitions are relevant to API designers, API Providers and API Consumers to organize their planning. The above table provides the complete set of actions that occur in every meta-release cycle.
...
| Concurrent support requirement by API deployments | API Providers MUST continue to deploy a Supported ICM version with the previous major ICM version number next to the newer major ICM version (until the end of the maintenance phase) | Applies to ICM-compatible API deployments |
...
NOTE: Patching of a Deprecated ICM version MUST be avoided as much as possible.
...
The published lifecycle state must be available in machine-readable form for the CAMARA validation support to consume ([CAMARA validation support](#65-camara-validation-support)). Until a schema is defined, the ICM version release notes are the single authoritative source — readable by humans but not by tooling.
...
In addition to the lifecycle state table, each ICM release MUST document **breaking changes** introduced in that ICM version, split by ICM design info changes (Table A) and ICM deployment info changes (Table B), covering the two aspects of ICM-compatibility ([ICM-compatibility](#3-icm-compatibility)). Non-breaking changes (additive features, clarifications) are documented in the regular CHANGELOG and do not require entries in the tables below.

**Table A — ICM design info - breaking changes** (impacting API version ICM-compatibility; see [API version ICM-compatibility](#31-api-version-icm-compatibility)). Used by API Sub Projects to assess whether their API versions need a new release.
...
**Table B — ICM deployment info - breaking changes** (impacting API deployment ICM-compatibility; see [API deployment ICM-compatibility](#32-api-deployment-icm-compatibility)). Used by API Providers and API Consumers to plan deployment updates.
...
This declared ICM version is the lowest ICM version on which the API definition relies and which is Supported at the time of public release of this API version.
...
When a new major ICM version is publicly released, the ICM-compatibility of each API version in the ICM-compatibility matrix with respect to this new ICM version needs to be assessed and agreed by governance:
- for API versions that do not rely on impacted ICM design info, based on positive API designer assessment (see below), ICM-compatibility is granted through an explicit governance decision. This "cross-major" continued ICM-compatibility is recorded as an annotation on the current pair in the ICM-compatibility matrix.
- for API versions that do rely on the impacted ICM design info, a new API version MUST be released. This new pair will be added to the ICM-compatibility matrix.

Example: `x-camara-min-icm: 1.2.0` declared in an API definition means that the API version depends on ICM design info from ICM version 1.2.0 onward. The definition is guaranteed ICM-compatible with any Supported or Deprecated ICM version ≥ 1.2.0 with the same major version number (e.g. with 1.2.1, 1.3.0, but not with 2.0.0). Whether an API version remains ICM-compatible with a subsequent major ICM version 2.0.0 is a governed assessment made at the time of the ICM 2.0.0 public version release.
...
This rule depends on the ICM lifecycle state being published in machine-readable form (see [Publication of lifecycle state](#541-publication-of-lifecycle-state)).

## 7. API deployment ICM-compatibility - details

This section describes the governance of API deployment ICM-compatibility.

An API definition might technically work against an ICM version that the compatibility matrix ([ICM-compatibility matrix](#9-icm-compatibility-matrix)) does not record as ICM-compatible — for example, a Retired or Revoked ICM version still running somewhere. Such pairings are outside CAMARA governance and are not supported by CAMARA.
...
The mechanism by which an API Provider provides multiple major ICM versions concurrently is an implementation choice and is not prescribed by this guideline; what matters is that each implemented ICM version is clearly announced by the API Provider for use by API Consumers during the relevant migration period. However, it is expected that an API Provider will run maximally 3 ICM versions in parallel: 1 Supported and 1 Deprecated ICM version in production, and 1 new Supported version in test. If required, the TSC can decide to put a cap on the number of ICM versions active in parallel lateron.
...
This section illustrates a typical scenario starting from Signal27, based on the ICM lifecycle state durations of the Supported maintenance window (24 months / 2 years) + the Deprecated maintenance window (12 months / 1 year) = total maintenance phase (36 months / 3 years).
...
Updated scenario (not fully done yet - needs cross-check):

- [**Signal27 ICM v1.0.0 Release**] Q2 2027: Signal27 released; ICM version in Supported state
- [**Sync27 API Release**] Q4 2027: Sync27 APIs released with Signal27 ICM version
- [**Signal27 ICM v1.0.0 Deployment**] Q4 2027-Q2 2029: earliest 6 months after ICM release and latest before Spring26 is Deprecated
- [**Sync27 API Deployment**] Q2 2028-Q2 2030: Earliest possible launch in production of Sync27 APIs with latest possible migration to Sync29 or Sync30 APIs by Retirement of Spring26 ICM version.
- [**Signal28 ICM v2.0.0 Release**] Q2 2028: Signal28 releases a new major ICM version
- [**Signal27 ICM v1.0.0 Supported (maintenance)**] Q2 2028: start of Signal27 ICM Supported maintenance window.
- [**Sync28 API Release**] Q4 2028: Sync28 APIs released with Signal28 ICM version
- [**Signal27 ICM v1.0.0 Deprecated**] Q2 2030: Signal27 ICM becomes Deprecated (2 years after start of maintenance in Q2 2028).
- [**Sync27 to Sync29/30 API Migration**] Q2 2030-Q2 2031: Sync27 APIs lose ICM-compatibility and MUST be retired or migrated. Migration can start earliest in Q2 2029 with Sync28 APIs, later in Q2 2030 with Sync29 APIs, or latest in Q2 2030 with Synch29 APIs. Recommended would be to migrate to Sync29 APIs. This will also depend on the choice of the supported ICM version (Signal28/29/30).
- [**Signal27 ICM v1.0.0 Retired**] Q2 2031: Signal27 ICM becomes Retired (3 years after start of maintenance in Q2 2028 period).
...
Note: The previous alternative with an ICM Supported duration of 18 months was rejected as too short, as, in that case ICM Sync27 maintenance would run Q2 2028 through Q4 2030 (retirement), and Sync30 APIs cannot realistically be the candidate for Sync27 API replacement because they have just been released, and operators typically want to avoid the potential risks of initial patch releases. They would be forced to migrate to Sync29 APIs instead and APIs would be actually deployed for maximally 30 months (2.5 years).
...
- **Scope**: impacted ICM version and lifecycle state before transition to Revoked, and the replacing Supported ICM versions SHALL be indicated.
- **Justification**: required — operational necessity, regulatory requirement, or security consideration.
- **Owner**: ICM team

These exceptions will appear in the ICM-compatibility matrix with an explicit annotation.
...
The ICM meeting of 2026-09-09: All issues have been covered.

1. **Exact durations** - covered - for the Supported and Deprecated state durations ([Duration of ICM lifecycle states](#53-duration-of-icm-lifecycle-states)). See Tanja's comment on section 8.3 (https://github.com/camaraproject/ReleaseManagement/pull/545#issuecomment-5121779848): the adopted ICM deployment lifespan (24 Supported + 12 Deprecated = 36 months (instead of previous 30 months)) provides an API deployment lifespan of 2 years (not 3). This was reconfirmed with operators in the ICM meeting of Aug. 26..
...
8. **Maximum number of concurrent non-Retired major ICM versions** - covered — whether to cap this to bound API Provider operational complexity when major ICM versions arrive in quick succession (for example, in a security-driven scenario), and how Retirement acceleration would be triggered if the cap is exceeded.
